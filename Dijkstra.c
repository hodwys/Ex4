...
pdijkstra getD(pdijkstra head, int id) 
{
    while (!(isEmpty(head))) 
    {
        if (head->node->id == id) {
...
pdijkstra getD_wet(pdijkstra head, int wet) 
{
    while (head != NULL) 
    {
        if(head->visit==0){
...
void deleteD(pdijkstra dijkstra) 
{
    while (dijkstra != NULL)
    {
...
int shortest_Path(pnode head, int src, int dest) 
{
    ...
    pdijkstra temp = getD_wet(dijkstraHead,0);
     ...
     while (!(isEmpty(temp))) 
    {
        ...
        {
            //faind dest
            pdijkstra n = getD(dijkstraHead, E_ind->endpoint->id);
