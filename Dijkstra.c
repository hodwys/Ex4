...
pdijkstra getDijkstra(pdijkstra head, int id) 
{
    while (!(isEmpty(head))) 
    {
        if (head->node->id == id) {
...
pdijkstra getDijkstra_wet(pdijkstra head, int wet) 
{
    while (head != NULL) 
    {
        if(head->visit==0){
...
int shortest_Path(pnode head, int src, int dest) 
{
    ...
    pdijkstra temp = getDijkstra_wet(dijkstraHead,0);
     ...
     while (!(isEmpty(temp))) 
    {
        ...
        {
            //faind dest
            pdijkstra n = getDijkstra(dijkstraHead, E_ind->endpoint->id);
        ...
        temp = getDijkstra_wet(dijkstraHead ,mini);
        //temp->visit=1;
        mini = INFINITY; 
        // temp = ..
       // temp = min(dijkstraHead);
    }
    int D = getDijkstra(dijkstraHead, dest)->weight;
