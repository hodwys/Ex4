...
pnode GetNoded( int id,pnode head ){
    pnode cur =head;
    while (!(isEmpty(cur)))
    {
        if(cur->id == id){
...
{
    // GET SRC
    pdijkstra dijkstraHead, temp = RunDijkstra(head, src);
   ...
   // pdijkstra temp = dijkstraHead;
    ...
    // {
    //     if (!dijkstraHead->visit && dijkstraHead->weight < INFINITY && (Node == NULL || Node->weight < dijkstraHead->weight)) 
    //     {
    //         Node = dijkstraHead;
    //     }
